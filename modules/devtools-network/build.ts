#!/usr/bin/env bun
/**
 * Builds `@rn-dev-modules/devtools-network` to `dist/index.js`. The module
 ...
 *
 * Run from repo root:   bun run modules/devtools-network/build.ts
 * Run via workspace:    bun run --filter @rn-dev-modules/devtools-network build
 */
...
if (!result.success) {
  console.error("devtools-network build failed:");
